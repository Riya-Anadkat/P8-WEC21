...
int main(){
    ...
    int indexBroken = -1;
    std::getline(f, nextInput);

    if(isdigit(nextInput[0])){
        indexBroken = (int) nextInput[0] - 48 - 2;
        //the index for the button that is broken has to be subtracted by 2 because our array starts at index 0
    }
